...
Этот каталог построен под календарь `лето -> отборы в октябре-ноябре -> финалы в марте-апреле`. Он не требует
решить все задачи подряд.

## Объём и маршрут

- этап A0: **50** задач - инженерная и алгоритмическая база;
- этап A1: **152** задачи - основные переносимые олимпиадные паттерны;
- этап B: **80** задач - регулярный финальный слой;
- этап C: **25** задач - выборочная продвинутая практика;
- полный каталог: **307** задач Codeforces/ACMP;
- основной маршрут без `H` и `X`: **233** задачи;
- LeetCode-база: **66** задач, в основной лимит не входит.
...
Роли: `D` - диагностика; `L` - изучение приема; `R` - закрепление; `H` - трудная задача; `F` - контрольная без
подсказок; `X` - сочетание тем. Для знакомой темы достаточно `D/F` и обязательного checkpoint; для новой темы
использовать `D/L/R/F`. `H/X` переносить на финальный цикл или брать по слабым местам. Точные лимиты попыток и правило
перехода дальше находятся в [ROADMAP](ROADMAP.md#adaptive-pace).

Колонка **`Паттерн`** описывает целевой учебный способ решения, а не утверждает, что других решений не существует.
Она специально оставлена видимой как карта навыков. Для диагностической или контрольной попытки ученик просто не
смотрит в эту колонку до истечения заранее выбранного времени.

## Правила работы

1. До начала темы решить `D` за ограниченное время, не читая колонку `Паттерн`. Если решение уверенное, перейти сразу
   к `F`; если идея не найдена, изучить теорию и перейти к `L`.
...
3. `F` решать как мини-контест: не читать колонку `Паттерн`, не использовать подсказки и старый код, провести полное
   тестирование и разбор после сдачи.
...
Этап **A0**. Задач: **10**. Теория и признаки распознавания: [ROADMAP: сложность и Java](ROADMAP.md#topic-complexity-java).
...
|   № | Задача                                                                          | Рейтинг CF | Роль | Паттерн                                                 |
| --: | ------------------------------------------------------------------------------- | ---------: | :--: | ------------------------------------------------------- |
|   1 | [ACMP 1 — A+B](https://acmp.ru/index.asp?main=task&id_task=1)                   |          — | `D`  | Java: запуск решения и базовый ввод/вывод               |
...
|   9 | [CF 118A — String Task](https://codeforces.com/problemset/problem/118/A)        |       1000 | `F`  | Линейная фильтрация строки без лишних объектов          |
|  10 | [CF 112A — Petya and Strings](https://codeforces.com/problemset/problem/112/A)  |        800 | `X`  | Нормализация регистра и лексикографическое сравнение    |
...
Этап **A0**. Задач: **10**. Теория и признаки распознавания: [ROADMAP: перебор](ROADMAP.md#topic-bruteforce).
...
|   № | Задача                                                                                     | Рейтинг CF | Роль | Паттерн                                                 |
| --: | ------------------------------------------------------------------------------------------ | ---------: | :--: | ------------------------------------------------------- |
|   1 | [CF 214A — System of Equations](https://codeforces.com/problemset/problem/214/A)           |        800 | `D`  | Полный перебор двух переменных по малым ограничениям    |
...
|   9 | [ACMP 101 — Магараджа](https://acmp.ru/index.asp?main=task&id_task=101)                    |          — | `F`  | Backtracking с rollback атакованных линий и клеток      |
|  10 | [CF 550B — Preparing Olympiad](https://codeforces.com/problemset/problem/550/B)            |       1400 | `X`  | Перебор подмножеств с несколькими ограничениями         |

<a id="practice-sorting-compression"></a>

## 3. Сортировка, компараторы и сжатие координат

Этап **A0**. Задач: **10**. Теория и признаки распознавания: [ROADMAP: сортировка](ROADMAP.md#topic-sorting-compression).
...
|   № | Задача                                                                           | Рейтинг CF | Роль | Паттерн                                                       |
| --: | -------------------------------------------------------------------------------- | ---------: | :--: | ------------------------------------------------------------- |
|   1 | [ACMP 41 — Сортировка подсчетом](https://acmp.ru/index.asp?main=task&id_task=41) |          — | `D`  | Counting sort на малом диапазоне значений                     |
...
|   9 | [CF 230A — Dragons](https://codeforces.com/problemset/problem/230/A)             |       1000 | `F`  | Сортировка объектов и последовательный инвариант достижимости |
|  10 | [CF 978F — Mentors](https://codeforces.com/problemset/problem/978/F)             |       1500 | `X`  | Сортировка/сжатие с дублями и возвратом к исходным индексам   |

<a id="practice-maps-sets"></a>

## 4. Частоты, HashMap/HashSet и множества

Этап **A0**. Задач: **9**. Теория и признаки распознавания: [ROADMAP: частоты и множества](ROADMAP.md#topic-maps-sets).
...
|   № | Задача                                                                                        | Рейтинг CF | Роль | Паттерн                                                  |
| --: | --------------------------------------------------------------------------------------------- | ---------: | :--: | -------------------------------------------------------- |
|   1 | [ACMP 82 — Пересечение множеств](https://acmp.ru/index.asp?main=task&id_task=82)              |          — | `D`  | Множество принадлежности и удаление дубликатов           |
...
|   8 | [ACMP 816 — Система пересекающихся множеств](https://acmp.ru/index.asp?main=task&id_task=816) |          — | `F`  | Двусторонние списки принадлежности множествам            |
|   9 | [CF 1108B — Divisors of Two Integers](https://codeforces.com/problemset/problem/1108/B)       |       1100 | `X`  | Мультимножество делителей и восстановление двух объектов |

<a id="practice-stack-queue-deque"></a>

## 5. Стек, очередь и дек

Этап **A0**. Задач: **6**. Теория и признаки распознавания: [ROADMAP: стек, очередь и дек](ROADMAP.md#topic-stack-queue-deque).

База LeetCode, не входит в лимит: [LC 20 - Valid Parentheses](https://leetcode.com/problems/valid-parentheses/) - стек для корректной вложенности.

|   № | Задача                                                                                             | Рейтинг CF | Роль | Паттерн                                                                  |
| --: | -------------------------------------------------------------------------------------------------- | ---------: | :--: | ------------------------------------------------------------------------ |
|   1 | [ACMP 899 - Баланс скобок](https://acmp.ru/index.asp?main=task&id_task=899)                        |          - | `D`  | Стек незакрытых скобок нескольких типов                                  |
|   2 | [CF 1234B2 - Social Network](https://codeforces.com/problemset/problem/1234/B2)                    |       1300 | `L`  | Ограниченная FIFO-очередь плюс `HashSet` присутствующих элементов        |
|   3 | [CF 1579E1 - Permutation Minimization by Deque](https://codeforces.com/problemset/problem/1579/E1) |       1000 | `L`  | Последовательный выбор `addFirst` или `addLast`                          |
|   4 | [CF 1907B - YetnotherrokenKeoard](https://codeforces.com/problemset/problem/1907/B)                |       1000 | `R`  | Два независимых стека индексов и восстановление исходного порядка        |
|   5 | [CF 1428C - ABBB](https://codeforces.com/problemset/problem/1428/C)                                |       1100 | `F`  | Потоковая редукция: новый символ взаимодействует только с вершиной стека |
|   6 | [CF 797C - Minimal string](https://codeforces.com/problemset/problem/797/C)                        |       1700 | `H`  | Стек-буфер плюс минимум необработанного суффикса                         |

Отдельный checkpoint, не входит в лимит задач: реализовать упрощенную версию `Расписания станков` - обычные задачи
добавляются в конец, срочные в начало, отмененные помечаются флагом и пропускаются только при достижении головы.

<a id="practice-priority-queue"></a>

## 6. Priority queue и heap

Этап **A0**. Задач: **5**. Теория и признаки распознавания: [ROADMAP: priority queue](ROADMAP.md#topic-priority-queue).

База LeetCode, не входит в лимит: [LC 703 - Kth Largest Element in a Stream](https://leetcode.com/problems/kth-largest-element-in-a-stream/) - heap из лучших `k` элементов.

|   № | Задача                                                                                | Рейтинг CF | Роль | Паттерн                                                               |
| --: | ------------------------------------------------------------------------------------- | ---------: | :--: | --------------------------------------------------------------------- |
|   1 | [CF 1800C2 - Powering the Hero](https://codeforces.com/problemset/problem/1800/C2)    |       1100 | `D`  | Добавление бонусов в max-heap и извлечение лучшего по событию         |
|   2 | [CF Gym 102961T - Room Allocation](https://codeforces.com/gym/102961/problem/T)       |          - | `L`  | Переиспользование ресурса с минимальным временем освобождения         |
|   3 | [CF 1353D - Constructing the Array](https://codeforces.com/problemset/problem/1353/D) |       1600 | `R`  | Составной comparator: длина по убыванию, левая граница по возрастанию |
|   4 | [CF 1468C - Berpizza](https://codeforces.com/problemset/problem/1468/C)               |       1700 | `F`  | Max-heap плюс FIFO/pointer, `served[]` и пропуск stale entries        |
|   5 | [CF 1526C2 - Potions](https://codeforces.com/problemset/problem/1526/C2)              |       1600 | `H`  | Tentative selection и удаление худшего выбранного элемента            |

<a id="practice-prefix-difference-2d"></a>

## 7. Префиксные суммы, массив разностей и 2D-префиксы

Этап **A1**. Задач: **7**. Теория и признаки распознавания: [ROADMAP: префиксы и разности](ROADMAP.md#topic-prefix-difference-2d).

База LeetCode, не входит в лимит: [LC 303 - Range Sum Query Immutable](https://leetcode.com/problems/range-sum-query-immutable/) - 1D-prefix; [LC 304 - Range Sum Query 2D Immutable](https://leetcode.com/problems/range-sum-query-2d-immutable/) - 2D-prefix.

|   № | Задача                                                                                   | Рейтинг CF | Роль | Паттерн                                                                           |
| --: | ---------------------------------------------------------------------------------------- | ---------: | :--: | --------------------------------------------------------------------------------- |
|   1 | [CF 1807D - Odd Queries](https://codeforces.com/problemset/problem/1807/D)               |        900 | `D`  | Префиксная сумма и виртуальная замена одного диапазона                            |
|   2 | [CF 816B - Karen and Coffee](https://codeforces.com/problemset/problem/816/B)            |       1400 | `L`  | Difference array для покрытия плюс второй prefix по предикату                     |
|   3 | [CF 1722E - Counting Rectangles](https://codeforces.com/problemset/problem/1722/E)       |       1600 | `R`  | Взвешенный 2D-prefix и строгие границы прямоугольного запроса                     |
|   4 | [CF 295A - Greg and Array](https://codeforces.com/problemset/problem/295/A)              |       1400 | `R`  | Два уровня offline difference arrays                                              |
|   5 | [CF 466C - Number of Ways](https://codeforces.com/problemset/problem/466/C)              |       1700 | `F`  | Подсчет упорядоченных пар точек разбиения по значениям префикса                   |
|   6 | [CF 1795C - Tea Tasting](https://codeforces.com/problemset/problem/1795/C)               |       1500 | `X`  | Binary search конца вклада, полные вклады через differences и один частичный край |
|   7 | [CF 276C - Little Girl and Maximum Sum](https://codeforces.com/problemset/problem/276/C) |       1500 | `X`  | Частоты покрытия через differences плюс перестановочный greedy                    |

<a id="practice-two-pointers"></a>

## 8. Два указателя

Этап **A1**. Задач: **5**. Теория и признаки распознавания: [ROADMAP: два указателя](ROADMAP.md#topic-two-pointers).

База LeetCode, не входит в лимит: [LC 11 - Container With Most Water](https://leetcode.com/problems/container-with-most-water/) - встречные указатели и доказанное отбрасывание кандидатов.

|   № | Задача                                                                            | Рейтинг CF | Роль | Паттерн                                                                                  |
| --: | --------------------------------------------------------------------------------- | ---------: | :--: | ---------------------------------------------------------------------------------------- |
|   1 | [CF Gym 102961G - Sum of Two Values](https://codeforces.com/gym/102961/problem/G) |          - | `D`  | Сортировка пар `(value, index)` и встречные указатели с восстановлением индексов         |
|   2 | [CF 1669F - Eating Candies](https://codeforces.com/problemset/problem/1669/F)     |       1100 | `L`  | Синхронизация положительных накопленных сумм с двух концов                               |
|   3 | [CF 489B - BerSU Ball](https://codeforces.com/problemset/problem/489/B)           |       1200 | `R`  | Greedy matching двух отсортированных групп                                               |
|   4 | [CF 1036D - Vasya and Arrays](https://codeforces.com/problemset/problem/1036/D)   |       1600 | `F`  | Синхронный проход по двум массивам и закрытие блоков при равенстве сумм                  |
|   5 | [CF 1538C - Number of Pairs](https://codeforces.com/problemset/problem/1538/C)    |       1300 | `X`  | Решить через линейный `count(sum <= x)` двумя указателями, не отдельными binary searches |

<a id="practice-sliding-window"></a>

## 9. Sliding window и monotonic deque

Этап **A1**. Задач: **7**. Теория и признаки распознавания: [ROADMAP: sliding window](ROADMAP.md#topic-sliding-window).

База LeetCode, не входит в лимит: [LC 3 - Longest Substring Without Repeating Characters](https://leetcode.com/problems/longest-substring-without-repeating-characters/) - частотное окно; [LC 239 - Sliding Window Maximum](https://leetcode.com/problems/sliding-window-maximum/) - monotonic deque.

|   № | Задача                                                                                     | Рейтинг CF | Роль | Паттерн                                                                                            |
| --: | ------------------------------------------------------------------------------------------ | ---------: | :--: | -------------------------------------------------------------------------------------------------- |
|   1 | [CF 1690D - Black and White Stripe](https://codeforces.com/problemset/problem/1690/D)      |       1000 | `D`  | Фиксированное rolling window: добавить справа и удалить вышедший символ                            |
|   2 | [CF 279B - Books](https://codeforces.com/problemset/problem/279/B)                         |       1400 | `L`  | Максимальное variable window с ограничением на сумму положительных элементов                       |
|   3 | [CF 701C - They Are Everywhere](https://codeforces.com/problemset/problem/701/C)           |       1500 | `R`  | Minimum-cover window с общей таблицей частот                                                       |
|   4 | [CF Gym 102961ZD - Subarray Distinct Values](https://codeforces.com/gym/102961/problem/ZD) |          - | `R`  | Подсчет подмассивов с `at most k` различными через `answer += r - l + 1`                           |
|   5 | [CF 6E - Exposition](https://codeforces.com/problemset/problem/6/E)                        |       1900 | `F`  | Самое длинное окно с `max-min <= k` строго через два monotonic deque, без `TreeSet` и segment tree |
|   6 | [CF 580B - Kefa and Company](https://codeforces.com/problemset/problem/580/B)              |       1500 | `X`  | Сортировка по ключу плюс variable window и `long`-сумма                                            |
|   7 | [CF 1358D - The Best Vacation](https://codeforces.com/problemset/problem/1358/D)           |       1900 | `H`  | Циклическое взвешенное окно и частично взятый край                                                 |

<a id="practice-monotonic-stack"></a>

## 10. Монотонный стек

Этап **A1**. Задач: **5**. Теория и признаки распознавания: [ROADMAP: монотонный стек](ROADMAP.md#topic-monotonic-stack).

База LeetCode, не входит в лимит: [LC 739 - Daily Temperatures](https://leetcode.com/problems/daily-temperatures/) - next greater; [LC 84 - Largest Rectangle in Histogram](https://leetcode.com/problems/largest-rectangle-in-histogram/) - границы области высоты.

|   № | Задача                                                                                 | Рейтинг CF | Роль | Паттерн                                                                  |
| --: | -------------------------------------------------------------------------------------- | ---------: | :--: | ------------------------------------------------------------------------ |
|   1 | [CF Gym 102961Z - Nearest Smaller Values](https://codeforces.com/gym/102961/problem/Z) |          - | `D`  | Previous strictly smaller; при равенстве требуется `pop >=`              |
|   2 | [CF 1886C - Decreasing String](https://codeforces.com/problemset/problem/1886/C)       |       1600 | `L`  | Greedy deletion через удаление больших символов с вершины                |
|   3 | [CF 547B - Mike and Feet](https://codeforces.com/problemset/problem/547/B)             |       1900 | `R`  | Ближайшие меньшие с двух сторон, span и распространение ответа по длинам |
|   4 | [CF 817D - Imbalanced Array](https://codeforces.com/problemset/problem/817/D)          |       1900 | `F`  | Contribution counting для `sum(max-min)` с асимметричными tie rules      |
|   5 | [CF 1313C2 - Skyscrapers](https://codeforces.com/problemset/problem/1313/C2)           |       1900 | `H`  | Stack DP для clipped prefix/suffix и восстановление оптимального массива |

<a id="practice-sweep-line"></a>

## 11. Sweep line

Этап **A1**. Задач: **5**. Теория и признаки распознавания: [ROADMAP: sweep line](ROADMAP.md#topic-sweep-line).

|   № | Задача                                                                                 | Рейтинг CF | Роль | Паттерн                                                                                |
| --: | -------------------------------------------------------------------------------------- | ---------: | :--: | -------------------------------------------------------------------------------------- |
|   1 | [CF 1000C - Covered Points Count](https://codeforces.com/problemset/problem/1000/C)    |       1700 | `D`  | Difference events на закрытых целочисленных интервалах и длины по числу покрытий       |
|   2 | [CF 612D - The Union of k-Segments](https://codeforces.com/problemset/problem/612/D)   |       1800 | `L`  | Равные координаты, порог покрытия и восстановление итоговых отрезков                   |
|   3 | [CF 1249D2 - Too Many Segments](https://codeforces.com/problemset/problem/1249/D2)     |       1800 | `R`  | Active set и greedy-удаление интервала с максимальной правой границей                  |
|   4 | [CF 1420D - Rescue Nibel](https://codeforces.com/problemset/problem/1420/D)            |       1800 | `F`  | Считать каждую группу в момент открытия последнего интервала; открытия раньше закрытий |
|   5 | [CF 1284D - New Year and Conference](https://codeforces.com/problemset/problem/1284/D) |       2100 | `H`  | Два симметричных sweep line и active multiset для проверки пересечений                 |

<a id="practice-binary-search"></a>

## 12. Бинарный/тернарный поиск и поиск по ответу

Этап **A1**. Задач: **10**. Теория и признаки распознавания: [ROADMAP: binary search](ROADMAP.md#topic-binary-search).
...
|   № | Задача                                                                                 | Рейтинг CF | Роль | Паттерн                                               |
| --: | -------------------------------------------------------------------------------------- | ---------: | :--: | ----------------------------------------------------- |
|   1 | [ACMP 267 — Ксерокопии](https://acmp.ru/index.asp?main=task&id_task=267)               |          — | `D`  | First true: минимальное время производства            |
...
|   9 | [CF 1742E — Scuza](https://codeforces.com/problemset/problem/1742/E)                   |       1200 | `F`  | Upper bound по максимумам префикса + сумма префикса   |
|  10 | [CF 1355E — Restorer Distance](https://codeforces.com/problemset/problem/1355/E)       |       2100 | `X`  | Дискретная унимодальность и тернарный поиск стоимости |

<a id="practice-greedy"></a>

## 13. Жадные алгоритмы, инварианты и обменный аргумент

Этап **A1**. Задач: **11**. Теория и признаки распознавания: [ROADMAP: greedy](ROADMAP.md#topic-greedy).
...
|   № | Задача                                                                            | Рейтинг CF | Роль | Паттерн                                                |
| --: | --------------------------------------------------------------------------------- | ---------: | :--: | ------------------------------------------------------ |
|   1 | [CF 34B — Sale](https://codeforces.com/problemset/problem/34/B)                   |        900 | `D`  | Выбор фиксированного числа наиболее выгодных элементов |
...
|  10 | [ACMP 39 — Волосатый бизнес](https://acmp.ru/index.asp?main=task&id_task=39)      |          — | `F`  | Суффиксный максимум и доказательство момента действия  |
|  11 | [CF 1365D — Solve The Maze](https://codeforces.com/problemset/problem/1365/D)     |       1700 | `X`  | Локальное жадное блокирование + проверка достижимости  |

<a id="practice-bitmasks"></a>

## 14. Биты, маски, подмаски и булева алгебра

Этап **A1**. Задач: **10**. Теория и признаки распознавания: [ROADMAP: биты и маски](ROADMAP.md#topic-bitmasks).
...
|   № | Задача                                                                                        | Рейтинг CF | Роль | Паттерн                                                   |
| --: | --------------------------------------------------------------------------------------------- | ---------: | :--: | --------------------------------------------------------- |
|   1 | [ACMP 542 — Бит-реверс](https://acmp.ru/index.asp?main=task&id_task=542)                      |          — | `D`  | Извлечение битов и построение числа сдвигами              |
...
|   9 | [CF 1516B — AGAGA XOOORRR](https://codeforces.com/problemset/problem/1516/B)                  |       1500 | `F`  | Префиксный XOR и разбиение на равные XOR-сегменты         |
|  10 | [CF 449D — Jzzhu and Numbers](https://codeforces.com/problemset/problem/449/D)                |       2400 | `X`  | SOS DP по маскам + inclusion–exclusion                    |

<a id="practice-number-theory"></a>

## 15. Теория чисел: gcd, простые, факторизация, решето

Этап **A1**. Задач: **13**. Теория и признаки распознавания: [ROADMAP: теория чисел](ROADMAP.md#topic-number-theory).
...
|   № | Задача                                                                                   | Рейтинг CF | Роль | Паттерн                                                    |
| --: | ---------------------------------------------------------------------------------------- | ---------: | :--: | ---------------------------------------------------------- |
|   1 | [ACMP 14 — НОК](https://acmp.ru/index.asp?main=task&id_task=14)                          |          — | `D`  | GCD/LCM и безопасный порядок умножения                     |
...
|  13 | [CF 710D — Two Arithmetic Progressions](https://codeforces.com/problemset/problem/710/D) |       2500 | `X`  | Линейные сравнения и CRT для пересечения прогрессий        |

<a id="practice-modular-combinatorics"></a>

## 16. Модульная арифметика и комбинаторика

Этап **A1**. Задач: **11**. Теория и признаки распознавания: [ROADMAP: модульная арифметика](ROADMAP.md#topic-modular-combinatorics).
...
|   № | Задача                                                                                           | Рейтинг CF | Роль | Паттерн                                                       |
| --: | ------------------------------------------------------------------------------------------------ | ---------: | :--: | ------------------------------------------------------------- |
|   1 | [ACMP 158 — Великий комбинатор](https://acmp.ru/index.asp?main=task&id_task=158)                 |          — | `D`  | Комбинаторная модель распределения с повторениями             |
...
|  10 | [CF 1305C — Kuroni and Impossible Calculation](https://codeforces.com/problemset/problem/1305/C) |       1600 | `F`  | Принцип Дирихле + произведение попарных разностей             |
|  11 | [CF 451E — Devu and Flowers](https://codeforces.com/problemset/problem/451/E)                    |       2300 | `X`  | Inclusion–exclusion по верхним ограничениям + сочетания       |

<a id="practice-basic-strings"></a>

## 17. Строки: префикс-функция, Z-функция и хеширование

Этап **A1**. Задач: **9**. Теория и признаки распознавания: [ROADMAP: базовые строки](ROADMAP.md#topic-basic-strings).
...
|   № | Задача                                                                             | Рейтинг CF | Роль | Паттерн                                                            |
| --: | ---------------------------------------------------------------------------------- | ---------: | :--: | ------------------------------------------------------------------ |
|   1 | [ACMP 202 — Поиск подстроки](https://acmp.ru/index.asp?main=task&id_task=202)      |          — | `D`  | KMP/Z: поиск всех вхождений образца                                |
...
|   8 | [CF 7D — Palindrome Degree](https://codeforces.com/problemset/problem/7/D)         |       2200 | `F`  | Rolling hash + DP по палиндромным префиксам                        |
|   9 | [CF 271D — Good Substrings](https://codeforces.com/problemset/problem/271/D)       |       1800 | `X`  | Trie или rolling hash для различных подстрок с ограничением        |
...
Этап **A1**. Задач: **8**. Теория и признаки распознавания: [ROADMAP: обходы графа](ROADMAP.md#topic-graph-traversals).
...
|   № | Задача                                                                               | Рейтинг CF | Роль | Паттерн                                             |
| --: | ------------------------------------------------------------------------------------ | ---------: | :--: | --------------------------------------------------- |
|   1 | [CF 217A — Ice Skating](https://codeforces.com/problemset/problem/217/A)             |       1200 | `D`  | Компоненты в неявно заданном графе                  |
...
|   7 | [CF 1702E — Split Into Two Sets](https://codeforces.com/problemset/problem/1702/E)   |       1600 | `F`  | Степени + двудольность графа из пар                 |
|   8 | [CF 377A — Maze](https://codeforces.com/problemset/problem/377/A)                    |       1600 | `X`  | DFS по сетке с сохранением связной части            |

<a id="practice-shortest-paths"></a>

## 19. Кратчайшие пути: BFS, 0-1 BFS, Дейкстра, Флойд, Беллман-Форд

Этап **A1**. Задач: **10**. Теория и признаки распознавания: [ROADMAP: кратчайшие пути](ROADMAP.md#topic-shortest-paths).
...
|   № | Задача                                                                                 | Рейтинг CF | Роль | Паттерн                                                 |
| --: | -------------------------------------------------------------------------------------- | ---------: | :--: | ------------------------------------------------------- |
|   1 | [ACMP 132 — Алгоритм Дейкстры](https://acmp.ru/index.asp?main=task&id_task=132)        |          — | `D`  | Dijkstra: релаксация и выбор минимального расстояния    |
...
|   9 | [CF 20C — Dijkstra?](https://codeforces.com/problemset/problem/20/C)                   |       1900 | `F`  | Dijkstra + parent[] + восстановление пути               |
|  10 | [CF 449B — Jzzhu and Cities](https://codeforces.com/problemset/problem/449/B)          |       2000 | `X`  | Dijkstra с несколькими типами стартовых рёбер           |

<a id="practice-trees-lca"></a>

## 20. Деревья: Эйлеров обход, двоичные подъёмы и LCA

Этап **A1**. Задач: **12**. Теория и признаки распознавания: [ROADMAP: деревья и LCA](ROADMAP.md#topic-trees-lca).
...
|   № | Задача                                                                                                                                      | Рейтинг CF | Роль | Паттерн                                                     |
| --: | ------------------------------------------------------------------------------------------------------------------------------------------- | ---------: | :--: | ----------------------------------------------------------- |
|   1 | [ACMP 141 — Дерево](https://acmp.ru/index.asp?main=task&id_task=141)                                                                        |          — | `D`  | Проверка структуры дерева                                   |
...
|  11 | [CF 519E — A and B and Lecture Rooms](https://codeforces.com/problemset/problem/519/E)                                                      |       2100 | `F`  | Binary lifting + размеры частей дерева                      |
|  12 | [CF 383C — Propagating tree](https://codeforces.com/problemset/problem/383/C)                                                               |       2000 | `X`  | Euler flatten + Fenwick с учётом чётности глубины           |
...
Этап **A1**. Задач: **13**. Теория и признаки распознавания: [ROADMAP: базовое DP](ROADMAP.md#topic-basic-dp).
...
|   № | Задача                                                                               | Рейтинг CF | Роль | Паттерн                                                         |
| --: | ------------------------------------------------------------------------------------ | ---------: | :--: | --------------------------------------------------------------- |
|   1 | [ACMP 11 — Зайчик](https://acmp.ru/index.asp?main=task&id_task=11)                   |          — | `D`  | Число способов и одномерная рекуррентность                      |
...
|  12 | [CF 706C — Hard problem](https://codeforces.com/problemset/problem/706/C)            |       1600 | `F`  | Два состояния строки, INF и переходы                            |
|  13 | [CF 864E — Fire](https://codeforces.com/problemset/problem/864/E)                    |       2000 | `X`  | 0/1 knapsack с дедлайнами и восстановлением набора              |
...
Этап **A1**. Задач: **5**. Теория и признаки распознавания: [ROADMAP: Fenwick](ROADMAP.md#topic-fenwick).

База LeetCode, не входит в лимит: [LC 307 - Range Sum Query Mutable](https://leetcode.com/problems/range-sum-query-mutable/) - point update и range sum.

|   № | Задача                                                                                     | Рейтинг CF | Роль | Паттерн                                                             |
| --: | ------------------------------------------------------------------------------------------ | ---------: | :--: | ------------------------------------------------------------------- |
|   1 | [ACMP 1084 - Дерево Фенвика](https://acmp.ru/index.asp?main=task&id_task=1084)             |          - | `D`  | Базовый `add`, prefix sum и range sum                               |
|   2 | [CF 652D - Nested Segments](https://codeforces.com/problemset/problem/652/D)               |       1800 | `L`  | Offline dominance: сортировка по одной границе и Fenwick по другой  |
|   3 | [CF 459D - Pashmak and Parmida's problem](https://codeforces.com/problemset/problem/459/D) |       1800 | `R`  | Преобразование элементов в частотные ранги и подсчет пар Fenwick-ом |
|   4 | [CF 1208D - Restore Permutation](https://codeforces.com/problemset/problem/1208/D)         |       1900 | `F`  | Поиск позиции по взвешенной префиксной сумме спуском по Fenwick     |
|   5 | [CF 61E - Enemy is weak](https://codeforces.com/problemset/problem/61/E)                   |       1900 | `H`  | Вклад среднего элемента в убывающие тройки через два направления    |

<a id="practice-segment-tree"></a>

## 23. Segment tree и lazy propagation

Этап **A1**. Задач: **6**. Теория и признаки распознавания: [ROADMAP: segment tree](ROADMAP.md#topic-segment-tree).

База LeetCode, не входит в лимит: [LC 715 - Range Module](https://leetcode.com/problems/range-module/) - динамическое покрытие диапазонов и lazy propagation.

|   № | Задача                                                                                    | Рейтинг CF | Роль | Паттерн                                                                   |
| --: | ----------------------------------------------------------------------------------------- | ---------: | :--: | ------------------------------------------------------------------------- |
|   1 | [ACMP 1185 - RMQ с изменением элемента](https://acmp.ru/index.asp?main=task&id_task=1185) |          - | `D`  | Point assignment и range maximum                                          |
|   2 | [CF 339D - Xenia and Bit Operations](https://codeforces.com/problemset/problem/339/D)     |       1700 | `L`  | Point update и merge, зависящий от уровня дерева                          |
|   3 | [CF 380C - Sereja and Brackets](https://codeforces.com/problemset/problem/380/C)          |       2000 | `R`  | Собственный ассоциативный узел для скобочной последовательности           |
|   4 | [CF 52C - Circular RMQ](https://codeforces.com/problemset/problem/52/C)                   |       2200 | `F`  | `range add + range min`, lazy tags и разбиение циклического диапазона     |
|   5 | [CF 242E - XOR on Segment](https://codeforces.com/problemset/problem/242/E)               |       2000 | `H`  | Побитовый составной узел и lazy range xor                                 |
|   6 | [CF 438D - The Child and Sequence](https://codeforces.com/problemset/problem/438/D)       |       2300 | `X`  | Амортизированное pruning по максимуму для modulo; это не стандартный lazy |

Обязательный checkpoint, не входит в лимит задач:

1. Реализовать обычный `range add + range min` на `[l, r)` с методами `apply`, `push` и `pull`.
2. Сверить point update, range query и lazy update с наивным массивом на случайных малых тестах.
3. Проверить композицию последовательностей `add/add`, `assign/assign`, `assign/add` и `add/assign`.

<a id="practice-static-rmq"></a>

## 24. Static RMQ и sparse table

Этап **A1**. Задач: **5**. Теория и признаки распознавания: [ROADMAP: static RMQ](ROADMAP.md#topic-static-rmq).

Перед задачами выполнить прямой drill: построить sparse table для минимума и проверить все непустые диапазоны на
случайных массивах сравнением с наивным ответом.

|   № | Задача                                                                                | Рейтинг CF | Роль | Паттерн                                                                                |
| --: | ------------------------------------------------------------------------------------- | ---------: | :--: | -------------------------------------------------------------------------------------- |
|   1 | [CF 1709D - Rorororobot](https://codeforces.com/problemset/problem/1709/D)            |       1700 | `D`  | Static range maximum плюс арифметическая достижимость                                  |
|   2 | [CF 1548B - Integers Have Friends](https://codeforces.com/problemset/problem/1548/B)  |       1800 | `L`  | GCD sparse table на соседних разностях и монотонный поиск границы                      |
|   3 | [CF 474F - Ant Colony](https://codeforces.com/problemset/problem/474/F)               |       2100 | `R`  | GCD/min диапазона плюс частота точного значения через списки позиций                   |
|   4 | [CF 359D - Pair of Numbers](https://codeforces.com/problemset/problem/359/D)          |       2000 | `F`  | Две static tables для `min` и `gcd`, binary search длины и восстановление всех ответов |
|   5 | [CF 689D - Friends and Subsequences](https://codeforces.com/problemset/problem/689/D) |       2100 | `X`  | Сравнить два решения: sparse table плюс binary searches и linear monotonic deques      |

<a id="practice-geometry"></a>

## 25. Вычислительная геометрия

Этап **B**. Задач: **7**. Теория и признаки распознавания: [ROADMAP: геометрия](ROADMAP.md#topic-geometry).
...
|   № | Задача                                                                                                                                  | Рейтинг CF | Роль | Паттерн                                                |
| --: | --------------------------------------------------------------------------------------------------------------------------------------- | ---------: | :--: | ------------------------------------------------------ |
|   1 | [ACMP 348 — Пересечение отрезков](https://acmp.ru/index.asp?main=task&id_task=348)                                                      |          — | `D`  | Orientation + point-on-segment + пересечение отрезков  |
...
|   6 | [CF 166B — Polygons](https://codeforces.com/problemset/problem/166/B)                                                                   |       2100 | `F`  | Строгое попадание выпуклого многоугольника без касаний |
|   7 | [CF Gym 101554D — Robert Hood](https://codeforces.com/gym/101554/problem/D) · [регистрация/отправка](https://codeforces.com/gym/101554) |          — | `X`  | Convex hull + rotating calipers для диаметра множества |

<a id="practice-advanced-graphs"></a>

## 26. DAG, топосортировка, SCC, мосты и точки сочленения

Этап **B**. Задач: **9**. Теория и признаки распознавания: [ROADMAP: продвинутые графы](ROADMAP.md#topic-advanced-graphs).
...
|   № | Задача                                                                                                                                       | Рейтинг CF | Роль | Паттерн                                                    |
| --: | -------------------------------------------------------------------------------------------------------------------------------------------- | ---------: | :--: | ---------------------------------------------------------- |
|   1 | [CF 510C — Fox And Names](https://codeforces.com/problemset/problem/510/C)                                                                   |       1600 | `D`  | Топологическая сортировка + невозможный префикс строк      |
...
|   8 | [CF 1000E — We Need More Bosses](https://codeforces.com/problemset/problem/1000/E)                                                           |       2100 | `F`  | Мосты + сжатие 2-edge-connected components + диаметр       |
|   9 | [CF 652E — Pursuit For Artifacts](https://codeforces.com/problemset/problem/652/E)                                                           |       2300 | `X`  | Bridge tree + агрегат наличия специального ребра на пути   |

<a id="practice-dsu-mst"></a>

## 27. DSU, MST и офлайн-связность

Этап **B**. Задач: **7**. Теория и признаки распознавания: [ROADMAP: DSU и MST](ROADMAP.md#topic-dsu-mst).
...
|   № | Задача                                                                                   | Рейтинг CF | Роль | Паттерн                                               |
| --: | ---------------------------------------------------------------------------------------- | ---------: | :--: | ----------------------------------------------------- |
|   1 | [ACMP 142 — Минимальный каркас](https://acmp.ru/index.asp?main=task&id_task=142)         |          — | `D`  | Минимальный остов: базовая реализация Kruskal         |
...
|   6 | [CF 1245D — Shichikuji and Power Grid](https://codeforces.com/problemset/problem/1245/D) |       1900 | `F`  | Prim на дополненном графе + восстановление объектов   |
|   7 | [CF 160D — Edges in MST](https://codeforces.com/problemset/problem/160/D)                |       2300 | `X`  | Kruskal по группам веса + bridges во временном графе  |

<a id="practice-advanced-dp"></a>

## 28. DP по отрезкам, решёткам, графам и деревьям

Этап **B**. Задач: **8**. Теория и признаки распознавания: [ROADMAP: продвинутое DP](ROADMAP.md#topic-advanced-dp).
...
|   № | Задача                                                                                | Рейтинг CF | Роль | Паттерн                                           |
| --: | ------------------------------------------------------------------------------------- | ---------: | :--: | ------------------------------------------------- |
|   1 | [CF 1528A — Parsa's Humongous Tree](https://codeforces.com/problemset/problem/1528/A) |       1600 | `D`  | Tree DP с двумя состояниями вершины               |
...
|   7 | [CF 721C — Journey](https://codeforces.com/problemset/problem/721/C)                  |       1800 | `F`  | DAG DP + parent + восстановление пути             |
|   8 | [CF 1092F — Tree with Maximum Cost](https://codeforces.com/problemset/problem/1092/F) |       1900 | `X`  | Rerooting: перенос взвешенной суммы по ребру      |

<a id="practice-subset-dp"></a>

## 29. DP по подмножествам, цифрам и профилю

Этап **B**. Задач: **7**. Теория и признаки распознавания: [ROADMAP: subset/digit/profile DP](ROADMAP.md#topic-subset-dp).
...
|   № | Задача                                                                          | Рейтинг CF | Роль | Паттерн                                                   |
| --: | ------------------------------------------------------------------------------- | ---------: | :--: | --------------------------------------------------------- |
|   1 | [CF 1036C — Classy Numbers](https://codeforces.com/problemset/problem/1036/C)   |       1900 | `D`  | Digit DP: position/tight/started и ограничение числа цифр |
...
|   6 | [CF 628D — Magic Numbers](https://codeforces.com/problemset/problem/628/D)      |       2200 | `F`  | Digit DP с tight, modulo и позиционным ограничением       |
|   7 | [CF 55D — Beautiful numbers](https://codeforces.com/problemset/problem/55/D)    |       2500 | `X`  | Digit DP с состоянием LCM ненулевых цифр                  |

<a id="practice-treap"></a>

## 30. Декартово дерево, treap и порядковые структуры

Этап **B**. Задач: **6**. Теория и признаки распознавания: [ROADMAP: treap](ROADMAP.md#topic-treap).
...
|   5 | [CF 702F — T-Shirts](https://codeforces.com/problemset/problem/702/F)                             |       2800 | `F`  | Treap/BST с агрегатами и lazy-изменениями                       |
|   6 | [CF 1748E — Yet Another Array Counting Problem](https://codeforces.com/problemset/problem/1748/E) |       2300 | `X`  | Cartesian tree + DP по поддеревьям                              |

<a id="practice-flows-matching"></a>

## 31. Потоки и паросочетания

Этап **B**. Задач: **7**. Теория и признаки распознавания: [ROADMAP: потоки](ROADMAP.md#topic-flows-matching).
...
|   № | Задача                                                                              | Рейтинг CF | Роль | Паттерн                                                       |
| --: | ----------------------------------------------------------------------------------- | ---------: | :--: | ------------------------------------------------------------- |
|   1 | [CF 120H — Brevity is Soul of Wit](https://codeforces.com/problemset/problem/120/H) |       1800 | `D`  | Двудольная модель и увеличивающие пути Куна                   |
...
|   6 | [CF 510E — Fox And Dinner](https://codeforces.com/problemset/problem/510/E)         |       2300 | `F`  | Flow/matching + восстановление циклов                         |
|   7 | [CF 1082G — Petya and Graph](https://codeforces.com/problemset/problem/1082/G)      |       2400 | `X`  | Maximum closure как min-cut                                   |

<a id="practice-advanced-strings"></a>

## 32. Ахо-Корасик, Манакер, suffix array/automaton

Этап **B**. Задач: **8**. Теория и признаки распознавания: [ROADMAP: продвинутые строки](ROADMAP.md#topic-advanced-strings).
...
|   № | Задача                                                                                                   | Рейтинг CF | Роль | Паттерн                                                           |
| --: | -------------------------------------------------------------------------------------------------------- | ---------: | :--: | ----------------------------------------------------------------- |
|   1 | [ACMP 70 — Степень строки](https://acmp.ru/index.asp?main=task&id_task=70)                               |          — | `D`  | Границы и период строки как база suffix-структур                  |
...
|   7 | [CF 710F — String Set Queries](https://codeforces.com/problemset/problem/710/F)                          |       2400 | `F`  | Динамический набор Aho–Corasick через логарифмические объединения |
|   8 | [CF 873F — Forbidden Indices](https://codeforces.com/problemset/problem/873/F)                           |       2400 | `X`  | Suffix automaton + агрегация по suffix links                      |

<a id="practice-advanced-trees"></a>

## 33. HLD, центроидная декомпозиция, small-to-large и rerooting

Этап **B**. Задач: **7**. Теория и признаки распознавания: [ROADMAP: продвинутые деревья](ROADMAP.md#topic-advanced-trees).
...
|   № | Задача                                                                               | Рейтинг CF | Роль | Паттерн                                            |
| --: | ------------------------------------------------------------------------------------ | ---------: | :--: | -------------------------------------------------- |
|   1 | [CF 1324F — Maximum White Subtree](https://codeforces.com/problemset/problem/1324/F) |       1800 | `D`  | Базовый rerooting с переносом лучшей суммы         |
...
|   6 | [CF 342E — Xenia and Tree](https://codeforces.com/problemset/problem/342/E)          |       2400 | `F`  | Centroid decomposition для динамического множества |
|   7 | [CF 375D — Tree and Queries](https://codeforces.com/problemset/problem/375/D)        |       2400 | `X`  | DSU-on-tree по частотам цветов в поддеревьях       |

<a id="practice-games"></a>

## 34. Теория игр: выигрыш/проигрыш, Nim и Sprague-Grundy

Этап **B**. Задач: **7**. Теория и признаки распознавания: [ROADMAP: теория игр](ROADMAP.md#topic-games).
...
|   № | Задача                                                                                          | Рейтинг CF | Роль | Паттерн                                                   |
| --: | ----------------------------------------------------------------------------------------------- | ---------: | :--: | --------------------------------------------------------- |
|   1 | [CF 1527B1 — Palindrome Game (easy version)](https://codeforces.com/problemset/problem/1527/B1) |       1200 | `D`  | Формальная классификация win/lose по инварианту состояния |
...
|   6 | [CF 768E — Game of Stones](https://codeforces.com/problemset/problem/768/E)                     |       2100 | `F`  | Sprague–Grundy, mex и XOR независимых компонент           |
|   7 | [CF 850C — Arpa and a game with Mojtaba](https://codeforces.com/problemset/problem/850/C)       |       2200 | `X`  | Sprague–Grundy по маскам степеней простых                 |

<a id="practice-meet-in-the-middle"></a>

## 35. Meet-in-the-middle и разбиение пространства поиска

Этап **B**. Задач: **7**. Теория и признаки распознавания: [ROADMAP: meet-in-the-middle](ROADMAP.md#topic-meet-in-the-middle).
...
|   № | Задача                                                                             | Рейтинг CF | Роль | Паттерн                                                 |
| --: | ---------------------------------------------------------------------------------- | ---------: | :--: | ------------------------------------------------------- |
|   1 | [CF 888E — Maximum Subsequence](https://codeforces.com/problemset/problem/888/E)   |       1800 | `D`  | Две половины subset sums + поиск дополнения             |
...
|   6 | [CF 585D — Lizard Era: Beginning](https://codeforces.com/problemset/problem/585/D) |       2300 | `F`  | MITM с восстановлением выбранных решений                |
|   7 | [CF 1257F — Make Them Similar](https://codeforces.com/problemset/problem/1257/F)   |       2400 | `X`  | Разбиение пространства битов + hash векторов расстояний |

<a id="practice-sqrt-mo"></a>

## 36. Корневая декомпозиция, Mo и офлайн-запросы

Этап **C**. Задач: **5**. Теория и признаки распознавания: [ROADMAP: sqrt и Mo](ROADMAP.md#topic-sqrt-mo).
...
|   4 | [CF 13E — Holes](https://codeforces.com/problemset/problem/13/E)                       |       2700 | `F`  | Sqrt decomposition с point updates и jump-агрегатами |
|   5 | [CF 455D — Serega and Fun](https://codeforces.com/problemset/problem/455/D)            |       2700 | `X`  | Динамическая последовательность в блоках + частоты   |

<a id="practice-rollback-persistence"></a>

## 37. Rollback, персистентность и динамическая связность

Этап **C**. Задач: **5**. Теория и признаки распознавания: [ROADMAP: rollback и persistence](ROADMAP.md#topic-rollback-persistence).
...
|   4 | [CF 1140F — Extending Set of Points](https://codeforces.com/problemset/problem/1140/F) |       2600 | `F`  | Segment tree over time + rollback DSU            |
|   5 | [CF 484E — Sign on Fence](https://codeforces.com/problemset/problem/484/E)             |       2500 | `X`  | Версии persistent segment tree по порогу         |

<a id="practice-dp-optimizations"></a>

## 38. Оптимизации DP: CHT/Li Chao, divide-and-conquer, Knuth

Этап **C**. Задач: **5**. Теория и признаки распознавания: [ROADMAP: оптимизации DP](ROADMAP.md#topic-dp-optimizations).
...
|   4 | [CF Gym 100212C — Order-Preserving Codes](https://codeforces.com/gym/100212/attachments/download/1727/20042005-winter-petrozavodsk-camp-andrew-stankevich-contest-10-en.pdf#page=4) · [регистрация/отправка](https://codeforces.com/gym/100212) |          — | `F`  | Knuth optimization и границы оптимального разбиения    |
|   5 | [CF 932F — Escape Through Leaf](https://codeforces.com/problemset/problem/932/F)                                                                                                                                                                |       2700 | `X`  | Tree DP + Li Chao/small-to-large                       |

<a id="practice-linear-algebra-fft"></a>

## 39. Матрицы, линейная алгебра, FFT/NTT

Этап **C**. Задач: **5**. Теория и признаки распознавания: [ROADMAP: линейная алгебра и FFT](ROADMAP.md#topic-linear-algebra-fft).
...
|   4 | [CF 1096G — Lucky Tickets](https://codeforces.com/problemset/problem/1096/G)             |       2400 | `F`  | Polynomial exponentiation через FFT/NTT                 |
|   5 | [CF 528D — Fuzzy Search](https://codeforces.com/problemset/problem/528/D)                |       2500 | `X`  | Несколько convolution для неточного сопоставления строк |

<a id="practice-probability-interactive"></a>

## 40. Вероятность, рандомизация, interactive и output-only

Этап **C**. Задач: **5**. Теория и признаки распознавания: [ROADMAP: probability и interactive](ROADMAP.md#topic-probability-interactive).
...
|   4 | [CF 148D — Bag of mice](https://codeforces.com/problemset/problem/148/D)                      |       1800 | `F`  | Probability DP по состояниям количества объектов      |
|   5 | [CF 1114E — Arithmetic Progression](https://codeforces.com/problemset/problem/1114/E)         |       2200 | `X`  | Randomized sampling внутри interactive-протокола      |
